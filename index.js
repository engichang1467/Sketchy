...
async function getRandomWords(wordCount) 
{
	words = []
	for (let i = 0; i < wordCount; i++) 
	{
		randomWord = randomPictionaryList()
		let wordLink = await fetch(`https://en.wikipedia.org/w/api.php?action=opensearch&format=json&search=${randomWord}`);
		let wordLinkJson = await wordLink.json();
		let wordDef = await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${randomWord}`)
		let wordDefJson = await wordDef.json();

		let word = {word: randomWord, definition: wordDefJson[0]['meanings'][0]['definitions'][0]["definition"], link: wordLinkJson[3][0]}
